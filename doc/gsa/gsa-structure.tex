...
\begin{table}[h!]
  \begin{tabularx}{\linewidth}{|l|X|}
    ...
    \hline \gsainst{deletenlm} & Finalizes the pseudo-variable
    \gsavar{nlm}. \\
    \hline \gsainst{createnlm} & Creates the pseudo-variable \gsavar{nlm}.\\
...
\begin{instruction}{extract-field}
  \info{Instruction}{Maccess}{Mfield}{Extracts a field from a
...
%-----------------------------------------------------------------------------
\begin{instruction}{insert-field}
  ...
  \begin{operands}
  \item \operand{Usable}{The current value of the variable to be
      referenced.  This can be a direct variable reference,
      \gsainst{access} or \gsainst{deref} instruction.}
  \item \operand{Usable}{The computed address of the referenced item.}
  \item \operand{ConstInt}{The offset, in bytes, of the item in the
      record.}
...
%-----------------------------------------------------------------------------
\begin{instruction}{extract-element}
  \info{Instruction}{Maccess}{Melement}{Extracts an element from an
  ...
  \begin{operands}
  \item \operand{Usable}{The current value of the variable to be indexed.}
...
\end{instruction}
%-----------------------------------------------------------------------------
\begin{instruction}{insert-element}
...
\begin{instruction}{access-deref}
  ...
  \begin{notes}
    The \gsainst{createnlm} operand allows a consistent view of
...
\begin{instruction}{update-deref}
  ...
  \begin{notes}
    The addition of \gsainst{createnlm} as an operand allows a
...
%-----------------------------------------------------------------------------
\begin{instruction}{deletenlm}
  \info{Instruction}{Mmemory}{Mdeletenlm}{Ensures that all non-local
    variables are written back to their home memory
   location}{Mdeletenlm.Mod}{deletenlm}{Mmemory}

  \begin{notes}
    \todo{The semantics of this instruction have not yet been
      implemented.}

    In conjunction with \refgsainst{createnlm}{Mmemory}, this
    instruction ensures that non-local variables are written back to
    their home memory location at appropriate times.
    
    Every \ac{greg} will have a \gsainst{deletenlm} in the
  ...
  \begin{seealso}
    \refgsainst{createnlm}{Mmemory}, \refgsavar{nlm},
    \refgsainst{exit}{Mregion}
  \end{seealso}
\end{instruction}

%-----------------------------------------------------------------------------
\begin{instruction}{createnlm}
  \info{Instruction}{Mmemory}{Mcreatenlm}{Models read and write
    access to non-local
    variables.}{Mcreatenlm.Mod}{createnlm}{Mmemory}

  \begin{notes}
    \gsavar{nlm} models \emph{nonlocal variables}, \emph{\byref
      parameters} and \emph{heap-based objects}.  This instruction,
    coupled with \refgsainst{deletenlm}{Mmemory} ensures that all
    such items are written back to their home memory location at the
    appropriate times.
    
    Every \ac{greg} will have a \gsainst{createnlm} instruction as
  ...
  \begin{seealso}
    \refgsainst{deletenlm}{Mmemory}, \refgsavar{nlm},
...
\begin{instruction}{exit}
  ...
  \voperands{
    \begin{enumerate}
    \item The result of \refgsainst{deletenlm}{Mmemory}.
    \end{enumerate}
...
\gsainst{condmerge} coalesces all the false regions of conjunctive and
...
\begin{instruction}{if}
  ...
  \begin{notes}
    ...
    conditional, an additional \refgsainst{condmerge}{Gmerge} can be
    present.
  ...
  \begin{seealso}
    \refgsainst{condmerge}{Gmerge}, \refgsainst{true}{Gguard},
    \refgsainst{false}{Gguard}
  \end{seealso}
\end{instruction}

%-----------------------------------------------------------------------------
\begin{instruction}{condmerge}
  \info{CondMerge}{Gmerge}{Gcond}{Models a control flow merge
  from a code path which failed a logical (\code{\&} and \code{OR})
  operation}{Gif.Mod}{condmerge}{Gmerge}
...
\end{verbatim}
    ...
    sense, the \gsainst{condmerge} is different than other merge
...
\begin{instruction}{loop}
  \info{Merge}{Gmerge}{Gloop}{Models the top and backedges of the
    looping construct.}{Gloop.Mod}{loop}{Gmerge}
...
\begin{instruction}{case}
  \info{Merge}{Gmerge}{Gcase}{Models the closing of a \code{CASE}
    statement.}{Gcase.Mod}{case}{Gmerge}
...
%-----------------------------------------------------------------------------
\begin{instruction}{put}
  \info{Instruction}{Hput}{\multisubcl{asS1, asU1, asS2, asU2, asS4,
      asU4}}{Models writing directly to
    memory.}{Hput.Mod}{put}{hardware}
...
\begin{table}[h!]
  \begin{tabularx}{\linewidth}{|l|X|}
    \hline Instruction & Description \\
    \hline \gsainst{diff} & Symmetric Difference \\
    \hline \gsainst{union} & Union \\
    \hline \gsainst{sub} & Subtraction \\
    \hline \gsainst{intersection} & Intersection \\
    \hline \gsainst{neg} & Negation \\
    \hline \gsainst{incl} & Inclusion \\
    \hline \gsainst{excl} & Exclusion \\
    \hline \gsainst{range} & Set constructor; integer range \\
    \hline \gsainst{element} & $\in$ test\\
    \hline \gsainst{convert} & Set constructor; individual element \\
    \hline
  \end{tabularx}
...
\begin{instruction}{diff}
  ...
  \begin{seealso}
    ...
    \refgsainst{set-intersection}{sarithmetic}
  \end{seealso}
\end{instruction}

%-----------------------------------------------------------------------------
\begin{instruction}{union}
...
%-----------------------------------------------------------------------------
\begin{instruction}{intersection}
  \info{Instruction}{Sintersection}{asU4}{Models \emph{set
      intersection}.}{Sintersection.Mod}{set-intersection}{sarithmetic}
